...
let Stop_Loss_Hit_or_Profit_booked = '-', Buy_Lock = 0;

let test1,buy_indication_for_file_when_buy_on_statent_display=0;
...
console.clear(); 
    ...
    if ((Down + 30) > cci_position && cci_position < -10) //< 0)
    {
      ...
      if (Buy_Lock == 0) {
        if (Pending_Transactions_status == 0) {
          ...
          Stop_Loss_Hit_or_Profit_booked = '-';
          test1 = 'in side ';
          Buy_Lock = 1;
          buy_or_not_buy_display_in_file("Buy");
        }
      }
    ...
    if ((Peak - 10) > Live_candles_data_input[499]) {
      if ((Buy + 50) < Live_candles_data_input[499]) //.... Only Profit Booked
      {
        if (Pending_Transactions_status == 1) {
          ...
          //Buy=0;
          Peak_reset = 0;
          Stop_Loss_Hit_or_Profit_booked = 'Stop Loss';
        }
      }
    }
    //------------------------------------------------------------------//
    if ((Peak - 20) > Live_candles_data_input[499]) 
    {
      if ((Buy + 50) > Live_candles_data_input[499])//.... Stop Loss hit
      {
        if (Pending_Transactions_status == 1) 
        {
          ...
          //Buy=0;
          Peak_reset = 0;
          Stop_Loss_Hit_or_Profit_booked = 'Profit Booked';
        }
      }
    }
    ...
    +Sell+" "+String(Last_Transaction_Profit).slice(0, 6)+" "+String(Total_Profit).slice(0, 6)+" "+Stop_Loss_Hit_or_Profit_booked+
    " "+d.getHours()+"-"+d.getMinutes()+"-"+d.getSeconds()+" "+d.getDate()+"/"+(d.getMonth()+1)+"/"+d.getFullYear()+" "+buy_or_not_buy_display_in_file()+'\n';
    //"+Date()+'\n';
    ...
    if (Stop_Loss_Hit_or_Profit_booked == 'Stop Loss') {
      console.log('Stop Loss Hit');
    }
    if (Stop_Loss_Hit_or_Profit_booked == 'Profit Booked') {
...
function buy_or_not_buy_display_in_file(s)
{
